...
function run {
	...
	START_TIME=$(date +%d-%m-%Y:%T)
        case $1 in
        spi) echo "Running main_spi at $START_TIME";
                    ./main_spi > /dev/null 2>&1 &
                    while true; do
                        count=$(ps -aux | grep main_spi | wc -l)
                        if [ $count -le 1 ];
                        then
                            break
                        fi
                        ...
                        ps -eo rss,pcpu,args \
                            ...
                            >> result.spi.cs.$START_TIME.csv
                    done
                    ;;
        i2c) echo "Running main_i2c at $START_TIME";
                    ./main_i2c > /dev/null 2>&1 &
                    while true; do
                        count=$(ps -aux | grep main_i2c | wc -l)
                        if [ $count -le 1 ];
                        then
                            break
                        fi
