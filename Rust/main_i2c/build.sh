...
then
    ...
    then
        rsync -P --rsh=ssh $(pwd)/target/armv7-unknown-linux-gnueabihf/release/main_i2c pi@192.168.8.141:~/skripsi/Rust/main_i2c
    else
        scp $(pwd)/target/armv7-unknown-linux-gnueabihf/release/main_i2c pi@192.168.8.141:~/skripsi/Rust/main_i2c
    fi
    echo "Operation done."
fi
